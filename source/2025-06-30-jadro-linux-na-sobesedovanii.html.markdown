...
Недавно друг задал довольно интересный вопрос: **а зачем на собеседованиях спрашивают как загружается Linux?** Что там можно интересное услышать? Ведь для любой ОС в целом это bios/uefi+бутлоадер+сама ОС. Да и работаешь ты уже с загруженной ОС, да и вообще не системный ты программист.

<p align="center">
<img src="/images/zatem.png" style="width: 50%">
</p>

Конечно, если вопрос у собеседующего появился из тетрадки с вопросами по Linux, то это история так себе. Частенько на такие вопросы нужно отвечать не глубоко и правильно, стараясь максимально показать степень насмотренности и продемонстрировать свой опыт, а так как написано в этой тетрадке.
...
Зачем такое спрашивать на собеседованиях DevOps? Вопрос тоже философский. В первую очередь, если это не специализированный инженер по редактированию yaml-файлов, то с инфраструктурой ему придётся работать 100%. Из чего вопрос про Linux плавно и вытекает.

Если покопаться поглубже, то вопрос загрузки ОС это, скорее, размышление о том, как ОС работает в рантайме на протяжении условного жизненного цикла и как хорошо (или плохо) работает в рантайме то, за что ты ответственен. В случае, например эмбеда, это может быть что угодно, от драйвера до системной приложухи в юзерспейсе. В случае инфрастуктуры задача примерно та же, только ты (в основном) не пишешь код сам, а запускаешь уже написанный. Так что, вопрос хорош для собеседований на много позиций: от embedded-разработки до инфраструктуры, system engineering и DevOps.
...
На этом этапе можно продемонстрировать знание GRUB, зачем ядру нужна cmdline и умение работать с UEFI Secure Boot (например, объяснить, как загружаются подписанные ядра, и что такое shim, MOK, и как ими управлять через mokutil).
...
Можно поговорить и про межпроцессное взаимодействие (IPC) — pipes, сокеты, очереди сообщений, сигналы, shm, mmap, и тд.
...
* как применял bpftrace, ftrace или eBPF-скрипты;
* что делал с out-of-memory (OOM) ситуациями, анализировал ли dmesg и логи ядра;
* использовал ли cgroups для ограничения процессов или debug-сценарии через systemd;
